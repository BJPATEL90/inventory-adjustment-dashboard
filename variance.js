// variance.js — V2: Add remark on ALL rows, REPLACE badge, facility type filter

var _varianceState = { allRows:[], filtered:[], showAll:false, facilityList:[], userList:[] };
  ...
  root.innerHTML =
    ...
    '<div class="card section-row">' +
      ...
      '<div class="table-wrap"><table><thead><tr>' +
        '<th>Date</th><th>Facility</th><th>Type</th><th>Brand</th><th>Username</th>' +
...
function _loadVarianceData(month) {
  apiGetVarianceData({ month: month||'' }).then(function(data) {
    ...
    document.getElementById('variance-tbody').innerHTML = '<tr><td colspan="13">'+emptyState('Error loading data',e.message||'Please try refreshing.')+'</td></tr>';
  });
}
...
function applyVarianceFilters() {
  ...
  var filtered = _varianceState.allRows.filter(function(row) {
    ...
    if (search) {
      var hay = [row.sku,row.itemName,row.username,row.facility,row.sourceRemark,row.userRemark].join(' ').toLowerCase();
...
function _renderVarianceTable(rows) {
  var tbody = document.getElementById('variance-tbody'); if (!tbody) return;
  if (!rows.length) { tbody.innerHTML='<tr><td colspan="13">'+emptyState('No records found','Try adjusting filters or toggling Variance Only.')+'</td></tr>'; return; }
  tbody.innerHTML = rows.map(function(row) {
    var statusCell;
    if (row.hasReplace) {
      statusCell = '<span class="badge badge-replace">⚠ REPLACE</span>';
    } else {
      statusCell = statusBadge(row.status);
    }
    var remarkCell;
    if (row.userRemark) {
    ...
    return '<tr'+(row.hasReplace?' class="replace-row"':'')+'>' +
      ...
      '<td><span class="badge badge-gray" style="font-size:10px;">'+row.facilityType+'</span></td>' +
