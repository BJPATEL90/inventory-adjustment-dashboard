// mtd.js — V4 FINAL
// Changes:
//   - Top 10 Users: table (Username | Variance Qty)
//   - Brandwise Breakdown + Facility Summary tables below charts
//   - MTD Top SKUs: SKU+Item combined cell, 4 columns (fixed colspan)
//   - Fixed missing + concatenation in thead
  ...
  root.innerHTML =
    ...
    '<div class="card section-row">' +
      ...
      '<div class="table-wrap">' +
        '<table><thead><tr>' +
          '<th style="width:40%">SKU / Item</th>' +
          '<th style="text-align:right">Added</th>' +
          '<th style="text-align:right">Removed</th>' +
          '<th style="text-align:right">Variance</th>' +
        '</tr></thead>' +
        '<tbody id="mtd-sku-tbody">' + skeletonRows(4, 8) + '</tbody>' +
        '</table>' +
      '</div>' +
    '</div>';
...
function _renderUserTable(data) {
  ...
  tbody.innerHTML = users.map(function(u, idx) {
    ...
    var vColor = vQty > 0 ? 'var(--orange)' : vQty < 0 ? '#DC2626' : 'var(--text-muted)';
    return '<tr>' +
...
function _renderMTDSKUTable(data) {
  ...
  if (!skus.length) {
    tbody.innerHTML = '<tr><td colspan="4">' + emptyState('No SKU data', 'No MTD SKU data available.') + '</td></tr>';
    return;
  }
