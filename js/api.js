...
function apiFetch(action, params, skipCache) {
  ...
  if (!skipCache && API._cache[cacheKey]) { var c=API._cache[cacheKey]; if(Date.now()-c.ts<API._cacheTTL) return Promise.resolve(c.data); }
  return new Promise(function(resolve,reject) {
    var cbName='__iamd_cb_'+(++API._callbackCounter)+'_'+Date.now(); var timeoutId;
    function cleanup(){ clearTimeout(timeoutId); try{delete window[cbName];}catch(e){} var s=document.getElementById(cbName); if(s&&s.parentNode)s.parentNode.removeChild(s); }
    timeoutId=setTimeout(function(){cleanup();reject(new Error('API timeout: '+action));},15000);
    window[cbName]=function(json){ cleanup(); if(!json||!json.success){reject(new Error(json&&json.error?json.error:'API error'));return;} if(!skipCache)API._cache[cacheKey]={ts:Date.now(),data:json.data}; resolve(json.data); };
    var qs=Object.assign({action:action,callback:cbName},params||{});
    var qs2=Object.keys(qs).map(function(k){return encodeURIComponent(k)+'='+encodeURIComponent(qs[k]!==null&&qs[k]!==undefined?qs[k]:'');}).join('&');
    var s=document.createElement('script'); s.id=cbName; s.src=API.BASE_URL+'?'+qs2;
    s.onerror=function(){cleanup();reject(new Error('Script load error: '+action));}; document.head.appendChild(s);
  });
}
